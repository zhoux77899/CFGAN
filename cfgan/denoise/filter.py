...
def filter_denoise(
    img: Union[np.ndarray, torch.Tensor], filter: "CrossFilter", eps: float = 1e-09
        ...
        **kwargs: float,
        ...
        if mask_types["i"]:
            if aspect_ratio == 1 and x_long_edge >= 3 * y_short_edge:
                ...
                for quad in ([1, 1], [0, 1], [1, 0], [0, 0]):
                    ...
                    mask[min(c_up, c_bottom) : max(c_up, c_bottom), min(c_left, c_right) : max(c_left, c_right)] = (
                        corner
                    )
                    mask[mask > 0] = 1
        ...
        if "gaussian_radius" in kwargs or "gaussian_sigma" in kwargs:
            ...
            gaussian_mask = scipy.ndimage.rotate(
                gaussian_mask, (x_rot + y_rot / aspect_ratio) / 2, reshape=False, order=0
            )
            mask *= gaussian_mask
