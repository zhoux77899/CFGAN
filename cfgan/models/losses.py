...
class GeneratorLoss(torch.nn.Module):
    ...
    def __init__(self, l1_weight: float, ssim_weight: float = 0, vgg_weight: float = 0, device: str = None) -> None:
    ...
    def l1_loss(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        loss_func = torch.nn.L1Loss()
        return loss_func(input, target)

    def ssim_loss(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        loss_func = pytorch_msssim.ssim
        return 1 - loss_func(input, target, data_range=1, size_average=True)

    def vgg_loss(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        # load VGG model
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        loss = 0.0 + torch.full((batch_size,), 0.0, dtype=torch.float32, device=self.device, requires_grad=True)
